Digital Literacy Portfolio — CSE0001
Student: Amrita Singh
Reg. No.: 25BCE10970
Branch: B.Tech – Computer Science Engineering
Year: First Year (2025–2026)
Institution: VIT Bhopal University
Course: CSE0001 – Digital Literacy
Project Overview
This repository is my Digital Literacy Portfolio for the CSE0001 course at VIT Bhopal University. The project is structured around the scenario of a Student Digital Ambassador — a peer resource for helping batchmates navigate the digital world responsibly and professionally.
REPOSITORY STUCTURE:
digital-literacy-project/
│
├── README.md                          ← This file
│
├── report/
│   └── Project_Report_Amrita_Singh.docx
│
├── task-1-presentation/
│   ├── description.md                 ← Tool used, design summary, reflection
│   └── infographic.png                ← Exported Canva infographic (add screenshot here)
│
├── task-2-portfolio/
│   ├── reflection.md                  ← Platforms, purpose, future plans
│   ├── github-profile.png             ← Screenshot of GitHub profile README
│   ├── linkedin-profile.png           ← Screenshot of LinkedIn Education section
│   └── kaggle-profile.png             ← Screenshot of Kaggle profile
│
├── task-3-platforms/
│   ├── platforms-reflection.md        ← HackerRank + Google Forms reflection
│   ├── hackerrank-challenge.png        ← Screenshot of completed challenge
│   ├── google-form.png                ← Screenshot of Digital Literacy Quiz form
│   └── google-sheet-responses.png     ← Screenshot of linked response Google Sheet
│
├── task-4-email-etiquette/
│   ├── email-drafts.md                ← Email 1 (extension request) + Email 2 (internship)
│   └── social-media-checklist.md      ← 5 Do's and 5 Don'ts for social media use
│
└── task-5-cybercrime/
    ├── casestudy.md                   ← UPI Fraud case study (200–250 words)
    └── prevention-checklist.md        ← 8+ actionable online safety tips
Module Summaries:
Task 1 – Digital Literacy Awareness Infographic
Created a one-page infographic using Canva covering: what digital literacy is, safe internet practices, and professional online presence. Exported as PNG and saved in task-1-presentation/.
Task 2 – Student Digital Portfolio
Set up foundational profiles on GitHub, LinkedIn, and Kaggle. Created a GitHub profile README with name, branch, year, and learning goals. Updated LinkedIn Education section. Screenshots saved in task-2-portfolio/.
Task 3 – Coding and Collaboration Platforms
Completed the "Solve Me First" challenge on HackerRank (Python 3). Built a 5-question Digital Literacy Awareness Quiz on Google Forms with at least one MCQ and one short-answer question. Screenshots and links in task-3-platforms/.
Google Form (Digital Literacy Quiz): https://docs.google.com/forms/d/e/1FAIpQLScbnO1-Ltq8wMys46xhu8LHPAKypVgVxi_Nr4zD4y4B6UhXbA/viewform?usp=publish-editor
Task 4 – Professional Email and Etiquette Guide
Drafted two professional emails: (1) requesting a deadline extension from a professor, and (2) expressing interest in a summer internship. Created a Social Media Do's and Don'ts checklist. All files in task-4-email-etiquette/.
Task 5 – Cybercrime Awareness (UPI/Online Payment Fraud)
Wrote a 200–250 word case study on the Fake Customer Care Refund Scam, a common UPI fraud pattern targeting college students. Created a prevention checklist with 8 actionable tips, 2 UPI-specific tips, and reporting channels (cybercrime.gov.in | Helpline 1930). Files in task-5-cybercrime/.  
